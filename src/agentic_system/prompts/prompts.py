...
request_summary_system = """{agent_descriptions}

You are summarizing a completed request for future runtime context.

Return a super-brief process summary in 1-2 sentences.
Focus on what the system did to solve the request.
Do not rewrite the final answer in full.
Do not use markdown, lists, or code fences.
If clarifications happened, mention them briefly only if they mattered.
Prefer mentioning the main delegated work and any important blocking issue or recovery if that shaped the outcome.
"""
...
history_window_summary_system = """{agent_descriptions}

You are compressing older completed user requests into rolling runtime memory.

Return a very compact plain-text summary.
Preserve only user intent and final outcome.
Mention important agents, tools, failures, or recoveries only when they materially affect future requests.
Do not include markdown, lists, code fences, or verbose audit detail.
If a previous summary exists, merge it with the new completed requests into one coherent updated summary.
"""
...
COMPLETED REQUESTS TO COMPRESS:
{completed_requests}
"""
